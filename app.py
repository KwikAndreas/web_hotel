...
def index():
    ...
    openDb()
    container = []
    sql = "SELECT * FROM orders ORDER BY INVOICE DESC;"
    cursor.execute(sql)
...
def generate_invoice():
    ...
    base_invoice_without_number = f"INV-{year_str}{current_month_str}"
    ...
    if last_invoice:
        ...
        next_invoice = f"INV-{str(next_number).zfill(3)}"
    else:
        next_number = 1
...
def order():
    ...
    if request.method == 'POST':
        ...
        openDb()
        sql = "INSERT INTO orders (invoice, nama_tamu, room_type, jumlah_tamu_dewasa, jumlah_tamu_anak, tambahan, total_harga, status, create_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"
        ...
        return render_template('order.html', invoice=invoice, tipe_ruangan=tipe_ruangan, fasilitas=fasilitas)
    

# Function to edit employee data
@application.route('/edit/<invoice>', methods=['GET', 'POST'])
def edit(invoice):
    openDb()
    cursor.execute('SELECT * FROM orders WHERE nik=%s', (invoice))
    ...
    if request.method == 'POST':
        ...
        sql = "UPDATE orders SET nama_tamu=%s, room_type=%s, jumlah_tamu_dewasa=%s, jumlah_tamu_anak=%s, tambahan=%s total_harga=%s, status=%s WHERE invoice=%s"
        val = (nama_tamu, room_type, jumlah_tamu_dewasa, jumlah_tamu_anak, ",".join(tambahan), total_biaya, invoice)
        cursor.execute(sql, val)
        conn.commit()
        closeDb()
        return redirect(url_for('index'))
...
def hapus(invoice):
    ...
    openDb()
    cursor.execute('DELETE FROM orders WHERE invoice=%s', (invoice,))
    conn.commit()
    closeDb()
    return redirect(url_for('index'))
...
def get_room_data(invoice):
    ...
    try:
        with connection.cursor() as cursor:
            sql = "SELECT * FROM orders WHERE invoice = %s"
